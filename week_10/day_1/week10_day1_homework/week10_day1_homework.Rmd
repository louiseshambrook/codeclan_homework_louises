...
```{r}
project_model <- lm(estimated_length ~ actual_length, project_management)

# estimated_length is a FUNCTION of actual, ie it depends on the actual length, hence why it is first (because R is a functional language, and states its intended output first)
```
...
Even then, before being able to check if we can rely on the p value, the QQ plot does not follow the line.
